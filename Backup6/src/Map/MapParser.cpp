...
bool MapParser::Load()
{
    //Load directory cua? 1 map co' duoi^ tmx (thuc. chat^' la` file XML)
    //Dat' ten^ cho map nay` la` level1
...
bool MapParser::Parse(std::string id, std::string source)
{
    ...
    GameMap* gamemap = new GameMap(); // Khai bao' 1 GameMap, GameMap  bao gom` nhieu` Layer
