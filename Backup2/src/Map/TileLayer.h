...
class TileLayer : public Layer
{
    ...
    private:
        int m_TileSize; //Kich' thuoc' cua? 1 tile
        int m_RowCount; //Tong? so' hang` trong 1 TileSet
        int m_ColCount; //Tong? so' cot^. trong 1 TileSet
        TileMap m_Tilemap; // 1 vector 2 chieu` luu ID cua cac' tile khac' nhau trong 1 Tileset
        TileSetsList m_Tilesets; //1 vector 1 chieu` luu cac' Tileset khac' nhau trong 1 TileLayer
};
