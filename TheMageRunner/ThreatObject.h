...
#include "BulletObject.h" //Phuc vu ham` InitBullet
...
public:
   ...
   //Trong ham` Show moi bat dau tinh' rect_.x va rect_.y
   ...
   //Ham` set va` get hidden live
   void set_hidden_live(const int& hid_live) { hidden_live_ = hid_live; }
   int get_hidden_live() const { return hidden_live_; }
   void DecreaseLive() { hidden_live_ --; }
   //Ham luu tru anh ke thu, chi load 1 lan
   void LoadAllThreatAction(SDL_Renderer* des);

private:

   //Xu ly = SDLCommonFunction
   //txt_threat_ deu la trung gian cua p_object_
   SDL_Texture* txt_threat_move_left_;
   SDL_Texture* txt_threat_move_right_;
   SDL_Texture* txt_threat_die_left_;
   SDL_Texture* txt_threat_die_right_;
