...
MainObject::~MainObject()
{
   ...
   if(txt_skill_barrier_ != NULL)
   {
       SDL_DestroyTexture(txt_skill_barrier_);
       txt_skill_barrier_ = NULL;
   }
}
...
{
   txt_skill_barrier_ = SDLCommonFunction::LoadTexture(g_name_skill_main_magic_barrier, des,
...
void MainObject::Show(SDL_Renderer* des)
{
   ...
   //Khi dang khong^ delay time de hoi sinh nhan vat
   if(come_back_time_ == 0)
   {
      ...
      {
          invin_time_--;
          static int fr_bar = 0; fr_bar++; fr_bar%=8;
          SDL_Rect current_clip = {200*fr_bar, 0, 200, 200};
          SDL_Rect renderQuad = {rect_.x - 30, rect_.y - 30, 200, 200};
          SDL_RenderCopy(des, txt_skill_barrier_, &current_clip, &renderQuad);
      }
      ...
      {
          pwup_time_--;
          renderQuad = {rect_.x, rect_.y, width_frame_*1.5, height_frame_*1.5};
      }
...
void MainObject::HandleInputAction(SDL_Event& events, SDL_Renderer* des)
{
   ...
   if(events.type == SDL_MOUSEBUTTONDOWN)
   {
      ...
      //Load img cua loai dan tuong ung
         ...
         if(pwup_time_ > 0)
         {
             pwup_bullet_mg_ = height_frame_;
             p_bullet->scale_width_height(3);
         }
         else
         {
             pwup_bullet_mg_ = 0;
         ...
         if(status_ == WALK_LEFT || status_ == IDLE_LEFT || status_ == JUMP_LEFT)
         {
            ...
            p_bullet->SetRect(this->rect_.x,
                              this->rect_.y + BULLET_MARGINS_Y_VAL - pwup_bullet_mg_);
            p_bullet->set_x_pos(this->x_pos_);
            p_bullet->set_y_pos(this->y_pos_+ BULLET_MARGINS_Y_VAL - pwup_bullet_mg_);
         }
         else if(status_ == WALK_RIGHT || status_ == IDLE_RIGHT || status_ == JUMP_RIGHT)
         {
...
void MainObject::CheckToMap(Map& map_data)
{
   ...
   if(pwup_time_ > 0)
   {
       width_ = width_frame_*1.5;
       height_ = height_frame_*1.5;
   }
   else
   {
       width_ = (width_frame_ < TILE_SIZE)? width_frame_ : TILE_SIZE;
       height_ = (height_frame_ < TILE_SIZE)? height_frame_ : TILE_SIZE;
   }
   ...
   **/
   ...
   {
      max_map_x = MAX_MAP_X_SCREEN;
      max_map_y = MAX_MAP_Y_SCREEN;
   }
   ...
   if(x1 >= 0 && x2 < max_map_x && y1 >= 0 && y2 < max_map_y)
   {
      if(x_val_ > 0) //Di chuyen sang phai
      {
         ...
         else if((GOLD_COIN <= val1 && val1 <= GOLD_COIN + GOLD_COIN_FRAME - 1)
                 || (GOLD_COIN <= val2 && val2 <= GOLD_COIN + GOLD_COIN_FRAME - 1) )
         {
             money_count_++;
            is_eat_item_ = true;
         }
         else if(val1 == HP_POTION || val2 == HP_POTION)
         {
             if(live_count_ < PLAYER_TOTAL_LIVES)
             {
                 live_count_++;
             }
            is_eat_item_ = true;
         }
         else if(val1 == HASTE_POTION || val2 == HASTE_POTION)
         {
            set_haste_time(10*SCREEN_FPS); //5s
            is_eat_item_ = true;
         }
         else if(val1 == INVUL_POTION || val2 == INVUL_POTION)
         {
            set_invin_time(10*SCREEN_FPS); //5s
            is_eat_item_ = true;
         }
         else if(val1 == POWER_POTION || val2 == POWER_POTION)
         {
            y_pos_ -= 80; // ~ 125*1.5 - 125
            set_pwup_time(10*SCREEN_FPS); //5s
            is_eat_item_ = true;
         }
         else
         {
            ...
            {
               x_pos_ = x2*TILE_SIZE;
               x_pos_ -= (width_ + EPSILON);
               x_val_ = 0;
            }
         }

         if(is_eat_item_ == true)
         {
             if(val1 != 43)
   ...
   **/
   ...
   //Vi. tri' cua nhan^ vat^ la o^ thu' bao nhieu^ sau khi di chuyen? theo phuong x
   x1 = (x_pos_)/TILE_SIZE;
   x2 = (x_pos_ + width_)/TILE_SIZE;

   //Vi. tri' cua nhan^ vat^ la o^ thu' bao nhieu^ sau khi di chuyen? theo phuong y
   y1 = (y_pos_ + y_val_)/TILE_SIZE;
   y2 = (y_pos_ + y_val_ + height_ - EPSILON) / TILE_SIZE;
   ...
   else if(x_pos_ + width_ > map_data.max_x_) //Nhan vat di chuyen cham toi bien phai?
   {
        x_pos_ = map_data.max_x_ - width_ - EPSILON;
   }
   //Nhan vat di chuyen cham toi bien duoi (roi xuong vuc)
   if(y_pos_ + height_ > map_data.max_y_)
   {
        live_count_--;
        y_pos_ = map_data.max_y_ - height_ - EPSILON;
        if(live_count_ < 0)
        {
            set_ko_time(5*SCREEN_FPS); //5s
        }
        else
        {
            set_come_back_time(60);
        }
   }
   else if(y_pos_ < 0)
   {
