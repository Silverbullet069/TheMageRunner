...
int SDLCommonFunction::ShowMenu(TTF_Font* font)
{
   g_menu = LoadImage("start_menu.png");
   if(g_menu == NULL)
   {
      return 1; //1 la chi so exit
   }
   ...
   while(true)
   {
      SDL_RenderCopy(g_renderer, g_menu, NULL, NULL);

      //In ra cac nut cua menu voi font truyen vao
      for(int i = 0 ; i < kMenuItemNum ; ++i)
      {
         text_menu[i].CreateGameText(font);
      }
      ...
      SDL_RenderPresent(g_renderer); // Update lien tuc Menu
   }
   return 1;
}
...
SDL_Texture* SDLCommonFunction::LoadImage(const std::string file_path)
{
   //Texture duoc load vao
   ...
   else
   {
      //Xoa phong
		...
		//Khoi tao texture tu surface
		new_texture = SDL_CreateTextureFromSurface(g_renderer, loaded_surface);

		//Neu khoi tao texture khong duoc
		if(new_texture == NULL)
      {
         printf( "Unable to create texture from %s! SDL Error: %s\n", file_path.c_str(), SDL_GetError() );
      }

      //Xoa surface cu, khong can dung den nua
...
SDL_Rect SDLCommonFunction::LoadWidthHeight(const std::string file_path)
{
   //Load image vao surface tai 1 dia chi cu the
   SDL_Surface* loaded_surface = IMG_Load(file_path.c_str());
   if(loaded_surface == NULL)
   ...
   {
      int w = loaded_surface->w;
      int h = loaded_surface->h;
      SDL_FreeSurface(loaded_surface);
      return {0, 0, w, h};
   }
}
   ...
   else
   {
      ...
      else
      {
         ...
         else
         {
            ...
            SDL_SetRenderDrawColor( g_renderer, 0xFF, 0xFF, 0xFF, 0xFF ); //white
...
void SDLCommonFunction::ShowTexture(SDL_Texture* final_texture)
{
      SDL_RenderCopy(g_renderer, final_texture, NULL, NULL);
