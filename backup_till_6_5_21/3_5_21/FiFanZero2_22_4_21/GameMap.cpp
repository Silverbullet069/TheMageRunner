...
void GameMap::LoadMap(const std::string file_name)
{
   ...
   if(file_in.is_open()) //Kiem tra mo thanh cong
   {
      ...
      game_map_.max_x_ = (game_map_.max_x_ + 1)*TILE_SIZE;// 100*128 = 12800
      game_map_.max_y_ = (game_map_.max_y_ + 1)*TILE_SIZE;// 12*128 = 1536
...
void GameMap::DrawMap(SDL_Renderer* des)
{
   ...
   for(int i = y1 ; i < y2 ; i += TILE_SIZE)
   {
      ...
      for(int j = x1 ; j < x2 ; j += TILE_SIZE)
      {
         ...
         //Khoi tao flag repeat tung` frame cho item
         static int repeat_fr = 0;
         //Reset spell book
         if(val == SPELL_BOOK + SPELL_BOOK_FRAME) // 29 (blank)
         {
            game_map_.tile[map_y][map_x] = SPELL_BOOK;
            repeat_fr = 0;
         }
         ...
         /** XU LY ITEM **/
            ...
            //Xu ly spell book tile
            if(SPELL_BOOK <= val && val <= SPELL_BOOK + SPELL_BOOK_FRAME - 1) //25 => 28
            {
               repeat_fr++;
               if(repeat_fr == SPELL_BOOK_FRAME + 1) // 5 lan in
               {
