...
int main(int argc, char* argv[])
{
   ...
   /** XU LY ANIMATION TRUOC KHI CHAY GAME (BACKSTORY) **/

   SDL_SetRenderDrawColor(g_renderer, 0, 0, 0, 0xFF);
   SDL_Rect total_screen = {0, 0, SCREEN_WIDTH, SCREEN_HEIGHT};
   SDL_RenderFillRect(g_renderer, &total_screen);
   SDL_RenderPresent(g_renderer);

   bool done_animation = false;
   while(!done_animation)
   {
      ...
      TextObject inter_mono_1;
      inter_mono_1.SetColor(WHITE);
      inter_mono_1.SetTextTotal("Who am I ?");
      inter_mono_1.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL);
      inter_mono_1.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_2;
      inter_mono_2.SetColor(WHITE);
      inter_mono_2.SetTextTotal("What is my name ?");
      inter_mono_2.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + TEXT_SPACING);
      inter_mono_2.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_3;
      inter_mono_3.SetColor(WHITE);
      inter_mono_3.SetTextTotal("Where do I come from?");
      inter_mono_3.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 2*TEXT_SPACING);
      inter_mono_3.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_4;
      inter_mono_4.SetColor(WHITE);
      inter_mono_4.SetTextTotal("Why can't I remember anything?");
      inter_mono_4.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 3*TEXT_SPACING);
      inter_mono_4.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_5;
      inter_mono_5.SetColor(WHITE);
      inter_mono_5.SetTextTotal("So many questions in my head");
      inter_mono_5.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 4*TEXT_SPACING);
      inter_mono_5.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_6;
      inter_mono_6.SetColor(WHITE);
      inter_mono_6.SetTextTotal(", but i can't have even one answer...");
      inter_mono_6.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 5*TEXT_SPACING);
      inter_mono_6.RenderTypingAnimation(g_renderer, g_event);

      ScreenEffect::FadeOutScreen(g_renderer, TOTAL_SCREEN, BLACK, g_event);

      TextObject inter_mono_7;
      inter_mono_7.SetColor(WHITE);
      inter_mono_7.SetTextTotal("The last thing I remember...");
      inter_mono_7.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL);
      inter_mono_7.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_8;
      inter_mono_8.SetColor(WHITE);
      inter_mono_8.SetTextTotal("is I, lying on the ground, in the middle of nowhere...");
      inter_mono_8.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 1*TEXT_SPACING);
      inter_mono_8.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_9;
      inter_mono_9.SetColor(WHITE);
      inter_mono_9.SetTextTotal("With my eyes opened widely");
      inter_mono_9.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 2*TEXT_SPACING);
      inter_mono_9.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_10;
      inter_mono_10.SetColor(WHITE);
      inter_mono_10.SetTextTotal("Skins crawled by the northern's chill");
      inter_mono_10.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 3*TEXT_SPACING);
      inter_mono_10.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_11;
      inter_mono_11.SetColor(WHITE);
      inter_mono_11.SetTextTotal("Metallic taste in my mouth");
      inter_mono_11.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 4*TEXT_SPACING);
      inter_mono_11.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_12;
      inter_mono_12.SetColor(WHITE);
      inter_mono_12.SetTextTotal("Fresh grass's scent tapping my nose");
      inter_mono_12.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 5*TEXT_SPACING);
      inter_mono_12.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_13;
      inter_mono_13.SetColor(WHITE);
      inter_mono_13.SetTextTotal("Winds howling through my ears");
      inter_mono_13.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 6*TEXT_SPACING);
      inter_mono_13.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_14;
      inter_mono_14.SetColor(WHITE);
      inter_mono_14.SetTextTotal("Nobody was there... so nobody knew what had happened...");
      inter_mono_14.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 7*TEXT_SPACING);
      inter_mono_14.RenderTypingAnimation(g_renderer, g_event);

      ScreenEffect::FadeOutScreen(g_renderer, TOTAL_SCREEN, BLACK, g_event);

      TextObject inter_mono_15;
      inter_mono_15.SetColor(WHITE);
      inter_mono_15.SetTextTotal("There is an old sage, he lives close the border of the Center Continent");
      inter_mono_15.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL);
      inter_mono_15.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_16;
      inter_mono_16.SetColor(WHITE);
      inter_mono_16.SetTextTotal("Everybody has come to meet him, seeking his wisdom");
      inter_mono_16.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 1*TEXT_SPACING);
      inter_mono_16.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_17;
      inter_mono_17.SetColor(WHITE);
      inter_mono_17.SetTextTotal("I have come too, the old guy looked nice by the way");
      inter_mono_17.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 2*TEXT_SPACING);
      inter_mono_17.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_18;
      inter_mono_18.SetColor(WHITE);
      inter_mono_18.SetTextTotal("but what he said, trembling me...");
      inter_mono_18.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 3*TEXT_SPACING);
      inter_mono_18.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_19;
      inter_mono_19.SetColor(WHITE);
      inter_mono_19.SetTextTotal("\"Young man, the answer you seek aren't here, your destiny even I can't predict it\"");
      inter_mono_19.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 4*TEXT_SPACING);
      inter_mono_19.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_20;
      inter_mono_20.SetColor(WHITE);
      inter_mono_20.SetTextTotal("\"Go to the Unname Cave, hidden by the mists on the Forgotten Continent\"");
      inter_mono_20.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 5*TEXT_SPACING);
      inter_mono_20.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_21;
      inter_mono_21.SetColor(WHITE);
      inter_mono_21.SetTextTotal("\"When the moonlight shines down on the sea, the path will open for you\"");
      inter_mono_21.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 6*TEXT_SPACING);
      inter_mono_21.RenderTypingAnimation(g_renderer, g_event);

      TextObject inter_mono_22;
      inter_mono_22.SetColor(WHITE);
      inter_mono_22.SetTextTotal("\"And then, the new era for mage will be born, capturing the sky with gods... \"");
      inter_mono_22.SetPosXY(TEXT_MARGINS_X_VAL, TEXT_MARGINS_Y_VAL + 7*TEXT_SPACING);
      inter_mono_22.RenderTypingAnimation(g_renderer, g_event);
