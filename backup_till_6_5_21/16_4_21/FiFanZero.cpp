...
int main(int argc, char* argv[])
{
   ...
   //Make PlayerPower
      ...
      player_power.LoadImg(g_name_life, g_renderer);
      player_power.Init();
   ...
   //Khai bao MainObject
      ...
      //Load texture cua MainObject
         ret = plane_object.LoadImg(g_name_main_obj, g_renderer);
         if(!ret) { printf("Failed to load plane object!"); return 0; }
   ...
   //Khai bao ExplosionObject
      //ExplosionObject khi MainObject no
         ...
         ret = exp_main.LoadImg(g_name_exp_main, g_renderer);
         exp_main.set_clip();
         SDL_Rect tmp = exp_main.GetRect();
         printf("%d %d", tmp.w, tmp.h);
         if(!ret) { printf("Failed to load exp main!"); return 0; }

      //ExplosionObject khi ThreatObject no
         ExplosionObject exp_threat;
         ret = exp_threat.LoadImg(g_name_exp_threat, g_renderer);
         exp_threat.set_clip();
         if(!ret) {  printf("Failed to load exp threat!"); return 0;  }
   ...
   //Khai bao nhieu ThreatObject
            ...
            //Xu ly lan lan luot tung ThreatObject
               ...
               if(p_threat != NULL)
               {
                  //Load texture cua ThreatObject
                     ret = p_threat->LoadImg(g_name_threat_obj, g_renderer);
                     if(!ret) {
                  ...
                  //Khoi tao cac trang thai cua vien dan vua duoc tao ra
                     p_threat->InitAmo(p_amo, g_renderer);
               }
            }
      ...
      //Thiet lap menu game
      g_menu = SDLCommonFunction::LoadTexture(g_name_menu, g_renderer);
      if(g_menu == NULL) { printf("Unable to load menu!"); return 0;}

      int ret_menu = SDLCommonFunction::ShowMenu(g_menu, g_font_menu, g_renderer);
      ...
      while(!is_quit)
      {
         ...
         while(SDL_PollEvent(&g_event))
         {
            ...
            plane_object.HandleInputAction(g_event, g_renderer);
         }
...
**/
      ...
      //Copy MainObject ra man hinh
            plane_object.Render(g_renderer);
      //Trang thai cac vien dan hien co cua MainObject
            plane_object.MakeAmo(g_renderer);
      ...
      //Xu ly cac ThreatObject
            for (int tt = 0 ; tt < NUM_THREATS ; ++tt)
            {
               ...
               if(p_threat != NULL)
               {
                  ...
                  //Trang thai cac vien dan hien co cua ThreatObject
                     p_threat->MakeAmo(SCREEN_WIDTH, SCREEN_HEIGHT, g_renderer);
            ...
            time_game.CreateGameText(g_font_text, g_renderer);
            ...
            mark_game.CreateGameText(g_font_text, g_renderer);
