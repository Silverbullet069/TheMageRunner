...
void ThreatObject::InitAmo(AmoObject* p_amo, SDL_Renderer* des)
{
   if(p_amo != NULL)
   {
      bool ret = p_amo->LoadImg(g_name_bullet_threat, des);
      if(ret)
      {
...
void ThreatObject::MakeAmo(const int& x_limit, const int& y_limit, SDL_Renderer* des)
{
   for(int i = 0 ; i < p_amo_list_.size() ; ++i)
   {
