...
void TypingScene::InitBackStory(SDL_Renderer* des, SDL_Event& events)
{
   ...
   for(int i = 0 ; i < TOTAL_INNER_MONO_SCENE_1 ; ++i)
   {
      ...
      inter_mono_.RenderTypingAnimation(des, events);
   }

   ScreenEffect::FadeOutScreen(des, TOTAL_SCREEN, BLACK, events);
   ...
   for(int i = 0 ; i < TOTAL_INNER_MONO_SCENE_2 ; ++i)
   {
      ...
      case 7 :
         inter_mono_tmp = "Nobody was there... so nobody knew what had happened...";
         break;
      }
      inter_mono_.Free();
      inter_mono_.SetColor(WHITE);
