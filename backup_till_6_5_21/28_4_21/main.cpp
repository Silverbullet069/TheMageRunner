...
std::vector<ThreatObject*> MakeThreatList()
{
   ...
   for(int i = 0 ; i < THREAT_TOTAL_NUM_EACH_TYPE ; ++i)
   {
      ...
      if(p_threat != NULL)
      {
         ...
         //Tao ra 2 vien dan, 1 vien dan ban ben trai, 1 vien dan ban ben phai
   ...
   ThreatObject* fire_wisp_idle = new ThreatObject[THREAT_TOTAL_NUM_EACH_TYPE];

   for(int i = 0 ; i < THREAT_TOTAL_NUM_EACH_TYPE ; ++i)
   {
      ThreatObject* p_threat = (fire_wisp_idle + i);
      if(p_threat != NULL)
      {
         //Viet them
   ...
   //Trong khi chuong trinh dang chay
      ...
      /** Bat dau xu ly ke dich */
         ...
         for(int i = 0 ; i  < threats_list.size() ; ++i)
         {
            ...
            /** Neu 1 trong 2 TH va cham xay ra, player mat 1 mang */
            ...
            //Threat da bi gan' trang thai die va check load het cac frame
               if(p_threat->get_frame() == THREAT_FRAME_NUM - 1 && p_threat->get_type_move() == ThreatObject::THREAT_DIE)
               {
                  p_threat->Free();
               ...
               for(int t = 0 ; t < threats_list.size() ; ++t)
               {
                  ...
                  if(threat_obj != NULL)
                  {
                     ...
                     if(bCol)
                     {
                        ...
                        if(p_bullet->get_bullet_type() == BulletObject::ICE_SHARD
                              && threat_obj->get_type_element() == ThreatObject::FIRE
                              && threat_obj->get_type_move() != ThreatObject::THREAT_DIE)
                        {
                           threat_obj->set_type_move(ThreatObject::THREAT_DIE);
                        ...
                        */
                        ...
                        /**
                        //Free threat va xoa khoi threats_list
                        threat_obj->Free();
