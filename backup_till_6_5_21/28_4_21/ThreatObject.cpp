...
void ThreatObject::DoThreat(Map& map_data)
{
   if(come_back_time_ == 0)
   {
      ...
      else if(type_move_ == ThreatObject::THREAT_DIE)
      {
         //
      }
...
void ThreatObject::FuseSet(SDL_Texture* txt, const int& w_fr, const int& h_fr)
{
   BaseObject::SetObject(txt);
   this->set_width_frame(w_fr);
   this->set_height_frame(h_fr);
   this->set_clips();
}
...
void ThreatObject::ImpMoveType(SDL_Renderer* des)
{
   ...
   {
      FuseSet(txt_threat_static_, width_frame_static_, height_frame_static_);
   }
   else
   {
      //ke thu` duoi mat dat
      if(on_ground_ == true)
      {
         if(x_pos_ > animation_b_) //ke thu di qua' bien phai
         {
            ...
            FuseSet(txt_threat_left_, width_frame_left_, height_frame_left_);
         ...
         else if(x_pos_ < animation_a_) //ke thu di qua' bien trai
         {
            ...
            FuseSet(txt_threat_right_, width_frame_right_, height_frame_right_);

            this->ResetBullet(des);
         }
         else if(type_move_ == THREAT_DIE)
         {
            FuseSet(txt_threat_die_, width_frame_die_, height_frame_die_);
         }

      }
      //Ke thu tren khong trung (luc khoi tao hoac hoi sinh sau khi roi xuong vuc)
      else
      {
         if(type_move_ = ThreatObject::MOVE_LEFT)
         {
...
void ThreatObject::InitBullet(BulletObject* p_bullet, SDL_Renderer* des)
{
   if(p_bullet != NULL)
   {
      ...
      else
      {
         ...
         p_bullet->SetRect(rect_.x + THREAT_BULLET_MARGINS_X_VAL, y_pos_ + THREAT_BULLET_MARGINS_Y_VAL);
...
void ThreatObject::ResetBullet(SDL_Renderer* des)
{
   for(int i = 0 ; i < bullet_list_.size() ; ++i)
   {
      ...
      if(p_bullet != NULL)
      {
         ...
         if(type_move_ == MOVE_RIGHT)
         {
            p_bullet->set_bullet_type(BulletObject::FIRE_BALL_WISP_RIGHT);
            p_bullet->LoadImgBullet(des);
         }
         else if(type_move_ == MOVE_LEFT)
         {
            p_bullet->set_bullet_type(BulletObject::FIRE_BALL_WISP_LEFT);
            p_bullet->LoadImgBullet(des);
         }
      }
   }
