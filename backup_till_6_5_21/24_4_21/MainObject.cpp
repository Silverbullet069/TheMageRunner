...
MainObject::MainObject()
{
   ...
   txt_player_idle_ = NULL;
   rect_player_idle_.x = 0;
   rect_player_idle_.y = 0;
   rect_player_idle_.w = 0;
   rect_player_idle_.h = 0;
   width_frame_idle_ = 0;
   height_frame_idle_ = 0;

   txt_player_walk_left_ = NULL;
   rect_player_walk_left_.x = 0;
   rect_player_walk_left_.y = 0;
   rect_player_walk_left_.w = 0;
   rect_player_walk_left_.h = 0;
   width_frame_left_ = 0;
...
MainObject::~MainObject()
{
   if(txt_player_idle_ != NULL)
   {
      SDL_DestroyTexture(txt_player_idle_);
      txt_player_idle_ = NULL;
   }
...
bool MainObject::LoadPlayerIdleImg(SDL_Renderer* des)
{
   txt_player_idle_ = SDLCommonFunction::LoadTexture(g_name_main_obj_idle, des); //Set texture
   rect_player_idle_ = SDLCommonFunction::LoadRect(g_name_main_obj_idle); //Set rect
   if(txt_player_idle_ != NULL)
   {
      /** LUU Y : DISTANCE_FRAME_IDLE va PLAYER_FRAME_NUM **/
      width_frame_idle_ = (rect_player_idle_.w - DISTANCE_FRAME_IDLE*(PLAYER_FRAME_NUM - 1))/PLAYER_FRAME_NUM;;
      height_frame_idle_ = rect_player_idle_.h;
      //Trang thai xuat hien ban dau nhan vat, khong co tac dung khi su dung SDLCommonFunction::SetObject()
      this->set_width_frame(width_frame_idle_);
      this->set_height_frame(height_frame_idle_);
   }
   else
   {
      printf("Unable to load player idle object!");
   }
   return txt_player_idle_ != NULL;
}

bool MainObject::LoadPlayerWalkLeftImg(SDL_Renderer* des)
{
   txt_player_walk_left_ = SDLCommonFunction::LoadTexture(g_name_main_obj_move_left, des); //Set texture
   rect_player_walk_left_ = SDLCommonFunction::LoadRect(g_name_main_obj_move_left); //Set rect
   if(txt_player_walk_left_ != NULL)
   {
      /** LUU Y : DISTANCE_FRAME_MOVE va PLAYER_FRAME_NUM **/
      width_frame_left_ = (rect_player_walk_left_.w - DISTANCE_FRAME_MOVE*(PLAYER_FRAME_NUM - 1))/PLAYER_FRAME_NUM;;
      height_frame_left_ = rect_player_walk_left_.h;
   }
   else
   {
      printf("Unable to load player walk left object!");
   }
   return txt_player_walk_left_ != NULL;
}

bool MainObject::LoadPlayerWalkRightImg(SDL_Renderer* des)
{
   txt_player_walk_right_ = SDLCommonFunction::LoadTexture(g_name_main_obj_move_right, des); //Set texture
   rect_player_walk_right_ = SDLCommonFunction::LoadRect(g_name_main_obj_move_right); //Set rect
   if(txt_player_walk_right_ != NULL)
   {
      /** LUU Y : DISTANCE_FRAME_MOVE va PLAYER_FRAME_NUM **/
      width_frame_right_ = (rect_player_walk_right_.w - DISTANCE_FRAME_MOVE*(PLAYER_FRAME_NUM - 1))/PLAYER_FRAME_NUM;;
      height_frame_right_ = rect_player_walk_right_.h;
   }
   else
   {
      printf("Unable to load player walk right object!");
   }
   return txt_player_walk_right_ != NULL;
}
...
void MainObject::set_clips(const int& dis_frame)
{
   if(width_frame_ > 0 && height_frame_ > 0) //Da duoc set
   {
      for(int i = 0 ; i < PLAYER_FRAME_NUM ; ++i)
      {
         frame_clip_[i].x = i*(width_frame_ + dis_frame);
         frame_clip_[i].y = 0;
   ...
   if(events.type == SDL_MOUSEBUTTONDOWN)
   {
      ...
      //Load img cua loai dan tuong ung
         ...
         if(status_ == WALK_LEFT)
         {
            ...
            p_bullet->SetRect(this->rect_.x + width_frame_ - BULLET_MARGINS_X_VAL,
                              this->rect_.y + height_frame_ - BULLET_MARGINS_Y_VAL);
         }
         else if(status_ == WALK_RIGHT || status_ == WALK_NONE)
         {
      ...
      //Set vi tri cua vien dan tren tile map
         ...
         p_bullet->set_y_pos(this->get_y_pos() + this->get_height_frame());
...
{
   //Khi su dung ham nay lan dau tien, thuc hien luu tru~ p_object_
   if(g_tmp_texture != NULL)  {  g_tmp_texture = BaseObject::GetObject();  }
   ...
   if(on_ground_ == true)
   {
      ...
      if(status_ == WALK_LEFT)
      {
         ...
         this->set_clips(DISTANCE_FRAME_MOVE);
         /**Moi lan thay doi hinh anh nhan vat la phai set lai width_frame va height_frame + set_clips vi`
         hinh anh nhan vat thay doi => width_frame_ va height_frame_ thay doi => clip thay doi */
      }
      ...
      {
         BaseObject::SetObject(txt_player_idle_);
         this->set_width_frame(width_frame_idle_);
         this->set_height_frame(height_frame_idle_);
         this->set_clips(DISTANCE_FRAME_IDLE);
      }
   }
...
void MainObject::InitPlayer()
{
   ...
   else //x_pos < 256 (Nv dang o vi tri xuat phat)
   {
      x_pos_ = 0;
   }
   y_pos_ = 0;
