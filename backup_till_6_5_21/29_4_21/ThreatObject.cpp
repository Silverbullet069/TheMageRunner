...
{
   txt_threat_left_ = SDLCommonFunction::LoadTexture("threat_left.png", des); //Set texture
   rect_threat_left_ = SDLCommonFunction::LoadRect("threat_left.png"); //Set rect
   if(txt_threat_left_ != NULL)
   {
      width_frame_ = rect_threat_left_.w/THREAT_FRAME_NUM;
      height_frame_ = rect_threat_left_.h;
   }
   else
   {
...
{
   txt_threat_right_ = SDLCommonFunction::LoadTexture("threat_right.png", des); //Set texture
   rect_threat_right_ = SDLCommonFunction::LoadRect("threat_right.png"); //Set rect
   if(txt_threat_right_ != NULL)
   {
      width_frame_ = rect_threat_right_.w/THREAT_FRAME_NUM;
      height_frame_ = rect_threat_right_.h;
   }
   else
   {
...
{
   txt_threat_static_ = SDLCommonFunction::LoadTexture("threat_static.png", des); //Set texture
   rect_threat_static_ = SDLCommonFunction::LoadRect("threat_static.png"); //Set rect
   if(txt_threat_static_ != NULL)
   {
      width_frame_ = rect_threat_static_.w/THREAT_FRAME_NUM;
      height_frame_ = rect_threat_static_.h;
   }
   else
   {
   ...
   else
   {
      if(on_ground_ == true) //neu nhu ke thu tren mat dat, thi moi di chuyen
      {
         ...
         else if(x_pos_ < animation_a_) //ke thu di qua' bien trai
         {
            ...
            //LoadImg("threat_right.png", des);

            //Viet them
            BaseObject::SetObject(txt_threat_right_);
         }
      }
      else //ke thu tren khong trung
      {
         if(input_type_.left_ == 1)
         {
            //LoadImg("threat_left.png", des);
...
void ThreatObject::InitBullet(BulletObject* p_bullet, SDL_Renderer* des)
{
   if(p_bullet != NULL)
   {
      ...
      if(ret)
      {
         ...
         p_bullet->SetRect(rect_.x + 5, y_pos_ + 10); //rect_.x la vi tri cua threat
      ...
      {
         printf("Unable to init bullet threat!");
      }
   }
}
